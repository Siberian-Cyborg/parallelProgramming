...
// NOLINTNEXTLINE
TEST(SIMDTest, TestPermute)
{
    auto c = _mm256_set_pd(3.0, 1.0, 2.0, 0.0);
    auto c = _mm256_set_pd(3.0, 1.0, 2.0, 0.0);
    ...
    auto cp = _mm256_permute2f128_pd(c, c, _MM_SHUFFLE(2, 0));
    EXPECT_DOUBLE_EQ(cp[0], 0.0);
    EXPECT_DOUBLE_EQ(cp[1], 1.0);
    EXPECT_DOUBLE_EQ(cp[2], 2.0);
    EXPECT_DOUBLE_EQ(cp[3], 3.0);
}

// NOLINTNEXTLINE
TEST(SIMDTest, TestHSumElements)
{
    auto a = _mm256_set_pd(4.0, 3.0, 2.0, 1.0);
    auto b = _mm256_set_pd(8.0, 7.0, 6.0, 5.0);

    auto ab = _mm256_hadd_pd(a, b);
    EXPECT_DOUBLE_EQ(ab[0], a[0]+a[1]);
    EXPECT_DOUBLE_EQ(ab[1], b[0]+b[1]);
    EXPECT_DOUBLE_EQ(ab[2], a[2]+a[3]);
    EXPECT_DOUBLE_EQ(ab[3], b[2]+b[3]);
}
