...
void Visualizer::PopulateImage(Body* b, double* imageArray, int numBodies)
{
    for (int index = 0; index < numBodies; index++) {
        ...
        if (x > PIXEL_SIZE && x < IMAGEWIDTH - PIXEL_SIZE && y > PIXEL_SIZE && y < IMAGEHEIGHT - PIXEL_SIZE) {
...
void Visualizer::ImageColorSetup(double x, double y, double vMag, double* imageArray)
{
    ...
    for (int i = -PIXEL_SIZE / 2; i < PIXEL_SIZE / 2; i++) {
        for (int j = -PIXEL_SIZE / 2; j < PIXEL_SIZE / 2; j++) {
            ...
            double cFactor = PARTICLE_BRIGHTNESS / (pow(exp(pow(PARTICLE_SHARPNESS * (xP + i - Visualizer::MeterToPixel(x, IMAGEWIDTH)), 2.0)) + exp(pow(PARTICLE_SHARPNESS * (yP + j - Visualizer::MeterToPixel(y, IMAGEHEIGHT)), 2.0)),
                                                        /*1.25*/ 0.75)
                                 + 1.0);
...
double Visualizer::RangeAdjuster(double x) { return fmax(fmin(x, 1.0), 0.0); }
...
#ifdef CONVERT_PPM
        char cmd[255];
        sprintf(cmd, "convert images/Step%05i.ppm images/Step%05i.jpg", frame, frame);
        system(cmd);
#endif
    }
    else {
